...
A command-focused mod starter that shows clean action parsing, runtime diagnostics, and small stateful examples you can expand into real tools.
...
Built jars are written to `build/libs/hytale-mod-template-commands-1.1.0.jar`, with matching sources and javadoc jars next to it.

## Commands
- `/hdcommandsmodstatus`: Shows runtime status for CommandsModPlugin.
- `/hdcommandsmoddemo`: Runs a demo action for CommandsModPlugin.
...
2. Copy `build/libs/hytale-mod-template-commands-1.1.0.jar` into your server `mods/` directory.
